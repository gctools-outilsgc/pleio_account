...
def get_user_attempts(request, get_usernamefunc, username=None):
    ...
    username = lower(username or get_username(request, get_usernamefunc))
    # get by IP
...
def get_attempt_cache_key(username):
    ...
    return "{0}:failed:username:{1}".format(def_config.CACHE_PREFIX, lower(username))

def get_blocked_cache_key(username):
    """ get the cache key by username """
    return "{0}:blocked:username:{1}".format(def_config.CACHE_PREFIX, lower(username))