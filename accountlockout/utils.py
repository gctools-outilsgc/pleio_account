...
from . import users
from .helper import users_helper, ip_helper, utils_helper

def is_already_locked(request, get_username=None, username=None):
    """Parse the username & IP from the request, and see if it's
    already locked."""
    user_blocked = users_helper.is_already_locked(request, username, get_username)
    ip_blocked = ip_helper.is_source_already_locked(ip_helper.get(request))
...
def lockout_response(request):
    ...
    username = users_helper.get_username_from_request(request)
    if def_config.LOCKOUT_TEMPLATE:
        ...
        utils_helper.send_blocked_email(request, username)
...
def add_login_attempt_to_db(request, login_valid,get_usernamefunc=None,username=None):
    ...
    username = username or users_helper.get_username(request, get_usernamefunc)
